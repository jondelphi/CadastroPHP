...
<body>
    ...
    <div class="container">
        ...
        function rota()
        {
            // 1. Obtém os componentes da URL. 
            // parse_url lida com a separação do caminho e da query string (?...)
            $url_components = parse_url($_SERVER['REQUEST_URI']);

            // 2. Obtém o caminho (ex: /produtos) e limpa as barras
            // O ?? '/' garante que, se o caminho não existir, usamos '/'
            $path = trim($url_components['path'] ?? '/', '/');

            // 3. Define a rota principal
            if ($path == '') {
                // Se o caminho estiver vazio (raiz do site), é 'home'
                $main_route = 'home';
            } else {
                // Pega apenas a primeira parte do caminho (ex: /produtos/lista vira 'produtos')
                // Isso mantém a lógica do seu código original.
                $partes = explode('/', $path);
                $main_route = $partes[0];
            }
